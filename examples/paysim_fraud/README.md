...
dataset (6,362,620 rows, 493 MB), using the `churn\_analysis` playbook.

The source is not included (it is a public Kaggle dataset); its SHA-256

is recorded in `manifest.json` under `source\_fingerprint`.
...
1\. \*\*`discover\_dataset.py data\\Fraud.csv`\*\* — profile, detected values

&#x20;  for low-cardinality columns, compatibility report. It showed only

&#x20;  1 of 8 playbooks qualified: no column met the `id\_column` bar
...
&#x20;  (`row\_number() OVER ()`), the standard move any pipeline makes.

&#x20;  Re-running discovery: 6 of 8 playbooks qualified.

3\. \*\*`run\_project.py`\*\* with `--playbook churn\_analysis` and an explicit
...
`audit\_log.jsonl` records every stage, gate, and rationale with IST

timestamps. `execution\_preview.md` is exactly what was approved.
