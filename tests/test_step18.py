...
from delivery_engine.model import (
    G3_Z_ALPHA_HALF,
    G3_Z_BETA,
    LEAKAGE_THRESHOLD,
    train_baseline,
...
class TestLeakageSentinel:
    def _leaky_csv(self, tmp_path: Path) -> Path:
        ...
        return _csv(
            tmp_path / "leak.csv",
            ["flag", "leaky_cat", "clean_num", "leaky_num", "clean_cat"],
            rows,
        )

    def test_planted_leaks_flagged_clean_features_not(self, tmp_path: Path) -> None:
        f = train_baseline(
            str(self._leaky_csv(tmp_path)),
            "flag",
            ["clean_num", "leaky_num"],
            ["leaky_cat", "clean_cat"],
        )
        ...
        assert flagged["leaky_cat"]["association"] == pytest.approx(1.0, abs=1e-6)
        assert flagged["leaky_num"]["measure"] == "abs_point_biserial"
        assert f["leakage_threshold"] == LEAKAGE_THRESHOLD

    def test_warning_never_gates_metrics_still_produced(self, tmp_path: Path) -> None:
        f = train_baseline(str(self._leaky_csv(tmp_path)), "flag", ["leaky_num"], ["leaky_cat"])
    ...
    def test_clean_dataset_has_no_warnings(self, tmp_path: Path) -> None:
        rows = [[1 if i % 3 == 0 else 0, i % 11, ("x", "y")[i % 2]] for i in range(240)]
...
class TestMde:
    def _segment_csv(self, tmp_path: Path, rows: int = 200) -> Path:
        ...
        return _csv(tmp_path / "seg.csv", ["customer_id", "converted", "segment", "spend"], out)

    def test_fisher_mde_matches_cohen_closed_form(self, tmp_path: Path) -> None:
        src = self._segment_csv(tmp_path)
        f = run_inference(str(src), "chi2_independence", "converted", ["segment"], [], 0.05)
    ...
    def test_mwu_mde_matches_normal_approximation(self, tmp_path: Path) -> None:
        src = self._segment_csv(tmp_path)
        f = run_inference(str(src), "mann_whitney", "converted", [], ["spend"], 0.05)
        t = f["tests"][0]
        ...
        n1 = t["groups"]["False"]
        n2 = t["groups"]["True"]
        expected = (Z_975 + Z_80) * math.sqrt((n1 + n2 + 1) / (3.0 * n1 * n2))
        assert t["mde"]["rank_biserial"] == pytest.approx(expected, abs=1e-6)

    def test_stricter_alpha_raises_the_mde(self, tmp_path: Path) -> None:
        src = self._segment_csv(tmp_path)
        f05 = run_inference(str(src), "chi2_independence", "converted", ["segment"], [], 0.05)
        f01 = run_inference(str(src), "chi2_independence", "converted", ["segment"], [], 0.01)
...
# ── G2 + G3 in the model stage (train_baseline) ──────────────────────────────


class TestModelStageG2G3:
    """G2 pseudoreplication and G3 MDE disclosures emitted by train_baseline."""

    def _binary_csv(self, tmp_path: Path, n: int = 80) -> Path:
        rows: list[list[Any]] = [
            [float(i % 13), ("a", "b")[i % 3 % 2], ("pos", "neg")[i % 2]] for i in range(n)
        ]
        return _csv(tmp_path / "bin.csv", ["num", "cat", "label"], rows)

    def test_g2_pseudoreplication_disclosure_in_model_findings(self, tmp_path: Path) -> None:
        f = train_baseline(str(self._binary_csv(tmp_path)), "label", ["num"], ["cat"])
        g2 = f["g2_pseudoreplication"]
        assert g2["gate"] is False
        assert "Forstmeier" in g2["reference"]
        assert g2["assumed_independent_units"] == f["n_test"]

    def test_g3_mde_in_model_findings_matches_cohen_formula(self, tmp_path: Path) -> None:
        f = train_baseline(str(self._binary_csv(tmp_path)), "label", ["num"], ["cat"])
        g3 = f["g3_minimum_detectable_effect"]
        assert g3["gate"] is False
        assert "Cohen" in g3["reference"]
        expected = round((G3_Z_ALPHA_HALF + G3_Z_BETA) / math.sqrt(g3["n_test"]), 6)
        assert g3["mde_cohen_h"] == pytest.approx(expected, abs=1e-6)
...
def _grouped_segment_csv(path: Path, rows: int = 200) -> Path:
    ...
    for i in range(rows):
        ...
        threshold = 8 if seg == "a" else 1
        conv = "yes" if i % 10 < threshold else "no"
        out.append([f"C-{i:05d}", f"H-{i % 40:03d}", conv, seg, 100.0 + i])
    return _csv(path, ["customer_id", "household", "converted", "segment", "spend"], out)
...
class TestEndToEnd:
    ...
    def test_g2_pseudoreplication_disclosed(self, tmp_path: Path) -> None:
        ...
        s = json.loads((out / "findings" / "stats.json").read_text("utf-8"))["findings"]
        indep = s["independence"]
        cols = [g["column"] for g in indep["grouping_candidates"]]
        assert "household" in cols
        hh = next(g for g in indep["grouping_candidates"] if g["column"] == "household")
    ...
    def test_g2_no_grouping_no_warning(self, tmp_path: Path) -> None:
        ...
        src = _csv(tmp_path / "flat.csv", ["customer_id", "converted", "segment", "spend"], rows)
        plan = _approved_plan(src)
        out = tmp_path / "pkg"
        run(plan, load_playbook(SEGMENT), RULES, out, approvals=APPROVALS)
        s = json.loads((out / "findings" / "stats.json").read_text("utf-8"))["findings"]
    ...
    def test_g5_manifest_fingerprints_the_source(self, tmp_path: Path) -> None:
        src = _grouped_segment_csv(tmp_path / "seg.csv")
        plan = _approved_plan(src)
        out = tmp_path / "pkg"
        run(plan, load_playbook(SEGMENT), RULES, out, approvals=APPROVALS)
    ...
    def test_g5_different_source_different_fingerprint(self, tmp_path: Path) -> None:
        s1 = _grouped_segment_csv(tmp_path / "a.csv")
        s2 = _grouped_segment_csv(tmp_path / "b.csv", rows=204)
        m1 = tmp_path / "p1"
        m2 = tmp_path / "p2"
        run(_approved_plan(s1), load_playbook(SEGMENT), RULES, m1, approvals=APPROVALS)
        run(_approved_plan(s2), load_playbook(SEGMENT), RULES, m2, approvals=APPROVALS)
        f1 = json.loads((m1 / "manifest.json").read_text("utf-8"))
        f2 = json.loads((m2 / "manifest.json").read_text("utf-8"))
        assert f1["source_fingerprint"]["sha256"] != f2["source_fingerprint"]["sha256"]

    def test_g6_limitations_section_reads_from_findings(self, tmp_path: Path) -> None:
        out = self._run(tmp_path)
    ...
    def test_g6_never_fabricates_absent_caveats(self, tmp_path: Path) -> None:
    ...
    def test_g4_handoff_carries_the_bias_checklist(self, tmp_path: Path) -> None:
        out = self._run(tmp_path)
        handoff = json.loads((out / "handoff_manifest.json").read_text(encoding="utf-8"))
        mgr = " ".join(c["check"] for c in handoff["team_handoff"]["manager"]["checks"])
        assert "survivorship" in mgr.lower()
        assert "denominator" in mgr.lower()
        assert "granularity" in mgr.lower()

    def test_reperformable_same_hash_across_runs(self, tmp_path: Path) -> None:
        src = _grouped_segment_csv(tmp_path / "seg.csv")
        plan = _approved_plan(src)
        ...
        d1 = json.loads((tmp_path / "p1" / "findings" / "stats.json").read_text("utf-8"))["sha256"]
        d2 = json.loads((tmp_path / "p2" / "findings" / "stats.json").read_text("utf-8"))["sha256"]
        assert d1 == d2
...
class TestHuntRegressions:
    def test_h5_vanished_source_is_an_audited_stop(self, tmp_path: Path) -> None:
