...
def _clean_profile() -> dict[str, Any]:
    return {
        ...
        "columns": [
            {
                "name": "row_id", "dtype": "BIGINT", "total": 6362620,
                "nulls": 0, "distinct": 6362620, "valid_ratio": 1.0,
                "completeness": 1.0, "case_variants": 0,
            },
            {
                "name": "type", "dtype": "VARCHAR", "total": 6362620,
                "nulls": 0, "distinct": 5, "valid_ratio": 1.0,
                "completeness": 1.0, "case_variants": 0,
            },
        ],
    }
...
def _clean_validate() -> dict[str, Any]:
    return {
        "results": [
            {"rule_id": "R01", "column": "row_id", "rule": "unique",
             "failures": 0, "detail": "value must be unique",
             "sample": []},
            {"rule_id": "R02", "column": "type", "rule": "allowed",
             "failures": 0, "detail": "", "sample": []},
        ],
        "rules_evaluated": 2,
        "total_exceptions": 0,
    }
...
def _messy_profile() -> dict[str, Any]:
    return {
        "dama_scores": {
            "accuracy": None, "completeness": 0.90, "consistency": 0.97,
            "timeliness": None, "uniqueness": 1.0, "validity": 0.88,
        },
        "columns": [
            {"name": "customer_id", "dtype": "BIGINT", "total": 10000,
             "nulls": 0, "distinct": 10000, "valid_ratio": 1.0,
             "completeness": 1.0, "case_variants": 0},
            {"name": "owner_team", "dtype": "VARCHAR", "total": 10000,
             "nulls": 1000, "distinct": 8, "valid_ratio": 0.90,
             "completeness": 0.90, "case_variants": 0},
        ],
    }
...
def _messy_validate() -> dict[str, Any]:
    return {
        "results": [
            {"rule_id": "R01", "column": "customer_id", "rule": "unique",
             "failures": 0, "detail": "", "sample": []},
            {"rule_id": "R02", "column": "owner_team",
             "rule": "not_null", "failures": 1000, "detail": "",
             "sample": []},
        ],
        "rules_evaluated": 2,
...
class TestPlantedValues:
    def test_clean_scores_render_as_percentages(self) -> None:
        html = build_report_html(
            _clean_profile(), _clean_validate(), "data/x.csv",
            "hh", "vv",
        )
    ...
    def test_not_scored_dimensions_never_render_as_zero(self) -> None:
        ...
        html = build_report_html(
            _clean_profile(), _clean_validate(), "s", "h", "v",
        )
    ...
    def test_messy_values_render_with_exact_counts(self) -> None:
        html = build_report_html(
            _messy_profile(), _messy_validate(), "s", "h", "v",
        )
        assert "90.0%" in html
        assert "88.0%" in html
    ...
    def test_provenance_digests_in_footer(self) -> None:
        html = build_report_html(
            _clean_profile(), _clean_validate(), "s",
            "PROFILEHASH123", "VALIDATEHASH456",
        )
...
class TestDeterminism:
    def test_byte_identical_on_repeat(self) -> None:
        a = build_report_html(_clean_profile(), _clean_validate(),
                              "s", "h", "v", "19 July 2026")
        b = build_report_html(_clean_profile(), _clean_validate(),
                              "s", "h", "v", "19 July 2026")
        assert a == b
    ...
    def test_generation_date_is_the_only_varying_part(self) -> None:
        ...
        base = build_report_html(_clean_profile(), _clean_validate(),
                                 "s", "h", "v", "01 January 2026")
        later = build_report_html(_clean_profile(), _clean_validate(),
                                  "s", "h", "v", "31 December 2026")
        # differ only by the date strings
        assert base != later
        assert base.replace("01 January 2026", "DATE") == \
            later.replace("31 December 2026", "DATE")
...
class TestInjectedNumbers:
    ...
    def _legit_tokens(
        self, profile: dict[str, Any], validate: dict[str, Any],
...
class TestColourSignalsState:
    def test_clean_uses_good_colour_not_warn(self) -> None:
        html = build_report_html(_clean_profile(), _clean_validate(),
                                 "s", "h", "v")
    ...
    def test_messy_uses_warn_colour(self) -> None:
        html = build_report_html(_messy_profile(), _messy_validate(),
                                 "s", "h", "v")
...
class TestReportFromPackage:
    def _seal(self, d: Path, profile: dict[str, Any],
              validate: dict[str, Any]) -> Path:
        import json
        pkg = d / "pkg"
        (pkg / "findings").mkdir(parents=True)
        (pkg / "findings" / "dq_profile.json").write_text(
            json.dumps({"findings": profile, "sha256": "PROFHASH",
                        "stage": "dq_profile"}), encoding="utf-8")
        (pkg / "findings" / "dq_validate.json").write_text(
            json.dumps({"findings": validate, "sha256": "VALHASH",
                        "stage": "dq_validate"}), encoding="utf-8")
        return pkg

    def test_writes_report_html_into_package(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_missing_findings_is_a_clean_error(
        self, tmp_path: Path
    ) -> None:
    ...
    def test_source_label_pulled_from_manifest(
        self, tmp_path: Path
    ) -> None:
        import json
        pkg = self._seal(tmp_path, _clean_profile(), _clean_validate())
...
class TestHuntRegressions:
    ...
    def test_h2_source_label_is_escaped(self) -> None:
        ...
        html = build_report_html(
            _clean_profile(), _clean_validate(),
            "<img src=x onerror=alert(1)>", "h", "v",
        )
